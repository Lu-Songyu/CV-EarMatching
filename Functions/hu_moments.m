...
% T=maketform('affine',[1 0 0;0.45 1 0;0 0 1]);
% R =makeresampler({'cubic','nearest'},'fill');
% image = imtransform(i,T,R);
% image = rgb2gray(i);
imshow(image);
% L = superpixels(image,500);
% f = drawrectangle;
% foreground = createMask(f,image);
% b1 = drawrectangle;
% b2 = drawrectangle;
% background = createMask(b1,image) + createMask(b2,image);
% BW = lazysnapping(image,L,foreground,background);
% imshow(image);
% imshow(labeloverlay(image,BW,'Colormap',[0 1 0]))
% image = BW;
...
hu_moments_vector = [I_one, I_two];
%     ,I_three,I_four,I_five,I_six,I_seven,I_eight];
